...
def get_args():
    ...
    if not args.interface:
        ...
        parser.error(
            "[-] Please include option -m or -c, use --help for more info.")
    return args


def change_mac(interface, new_macaddress):
    """Change MAC address."""
    print("[+] Changing MAC adddress for " +
          interface + " to " + new_macaddress)
...
def get_current_mac(interface):
    ...
    ifconfig_result = ifconfig_result.decode(
        "utf-8")  # change bytecode object to string
    mac_address_result = re.search(
...
def main():
    ...
    if options.interface:
        if options.macaddress:
            ...
            current_mac = get_current_mac(options.interface[0])
            if current_mac == options.macaddress[0]:
